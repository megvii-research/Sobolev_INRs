...
def write_train_summary(data, output, writer, epoch, out_train_imgs_dir):
    ...
    x_plot = data['downsampled_coordinate'][start_index:end_index].detach().cpu().numpy()
    ...
    fig.savefig(os.path.join(out_train_imgs_dir, '%05d.%s' % (epoch, SAVE_FORMAT)), format=SAVE_FORMAT)
    plt.close(fig)
...
def write_test_summary(data, output, writer, epoch, out_test_imgs_dir, factor, filename):
    prefix = 'test_'
    ...
    zoom_x_plot = data['coordinate'][zoom_start_index:zoom_end_index].detach().cpu().numpy()
    ...
    fig.savefig(os.path.join(out_test_imgs_dir, '%05d.%s' % (epoch, SAVE_FORMAT)), format=SAVE_FORMAT)
    plt.close(fig)
