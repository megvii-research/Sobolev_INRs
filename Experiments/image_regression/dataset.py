...
def get_data(data_root, dataset, filename, is_gray, factor, der_operator):
    ...
    if (img.height % factor) or (img.width % factor):
        raise ValueError(
            "The width/height of image must be an integer multiple of factor!"
        )
    if is_gray:
        img = img.convert("L")
    ...
    grad = kornia.filters.spatial_gradient(
        img.unsqueeze(0), mode=der_operator, order=1, normalized=True
    ).squeeze(
        0
    )  # (c, 2, h, w), grad_x = grad[:, 0], grad_y = grad[:, 1]
    ...
    downsampled_img = img[
        :, ::factor, ::factor
    ].clone()  # equal to F.interpolate with mode='nearest'
    # downsampled_img = F.interpolate(img.unsqueeze(0), scale_factor=1/factor, mode='nearest').squeeze(0)
    downsampled_img_shape = namedtuple("shape", "height width")(
        downsampled_img.shape[1], downsampled_img.shape[2]
    )
    ...
    grad = einops.rearrange(
        grad, "c d h w -> (h w) (c d)"
    )  # (h*w, c*2), if c=3: (dx_r, dy_r, dx_g, dy_g, dx_b, dy_b)
    ...
    downsampled_coordinate = einops.rearrange(
        downsampled_coordinate, "h w c -> (h w) c"
    )
