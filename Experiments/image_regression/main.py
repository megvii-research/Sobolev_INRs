...
def config_parser():
    ...
    parser.add_argument('--num_epochs', type=int, default=50000, help="Number of epochs to train for.")

    # experiment options 
    parser.add_argument('--exp_name', type=str, default='supervision_val_der', help="Name of experiment.")
    ...
    # dataset options 
    parser.add_argument('--data_root', type=str, default='../../data', help="Root path to image datasets.")
    parser.add_argument('--dataset', type=str, default='Set5', choices=('Set5', 'DIV2K_valid'))
    parser.add_argument('--filename', type=str, help="Name of image file.")
    parser.add_argument('--factor', type=int, default=4, choices=(1, 2, 3, 4), help="Factor of downsampling.")
...
def train(args, model, data, epochs, lrate, epochs_til_summary, epochs_til_checkpoint, logging_dir, train_summary_fn, test_summary_fn, log_f):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if (not epoch % epochs_til_summary) or (epoch == epochs):
            ...
            # training summary
            psnr, ssim = train_summary_fn(data, model_output, writer, epoch, downsampled_img_shape, out_train_imgs_dir)
            ...
            with torch.no_grad():
                ...
                psnr, ssim = test_summary_fn(data, model_output, writer, epoch, img_shape, out_test_imgs_dir, factor=args.factor)
...
def main():
    ...
    with open(os.path.join(logging_dir, 'log.txt'), 'w') as log_f:
        ...
        data = get_data(args.data_root, args.dataset, args.filename, args.is_gray, args.factor, args.der_operator)
