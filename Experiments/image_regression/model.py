...
class MLP(nn.Module):
    def __init__(self, in_features, w0, activations, out_features, hidden_features, num_hidden_layers, has_pos_encoding, has_fourier_feature, shape, sidelength):
        super().__init__()
        ...
        activations_and_inits = {
                'sine':(Sine(self.w0), first_layer_sine_init, partial(sine_init, w0=self.w0)),
        ...
        net = []
        net.append(nn.Sequential(nn.Linear(in_features=in_features, out_features=hidden_features), activations_and_inits[self.activations[0]][0])) # input layer
        for i in range(num_hidden_layers): # hidden layers
            net.append(nn.Sequential(nn.Linear(in_features=hidden_features, out_features=hidden_features), activations_and_inits[self.activations[i + 1]][0]))
            
        net.append(nn.Sequential(nn.Linear(in_features=hidden_features, out_features=out_features))) # output linear layer, without activation
        ...
        for i in range(self.num_hidden_layers): # hidden layers
            self.net[i + 1].apply(activations_and_inits[self.activations[i + 1]][2]) # following layer
        self.net[-1].apply(activations_and_inits[self.activations[-1]][2]) # output layer, initialize as hidden layers
    ...
    def normalize_coordinate(self, coordinate, shape):
        ...
        normalized_coordinate *= 2. # [-0.5 ~ 0.5] to [-1., 1.]
    ...
    def forward(self, input_coordinate, mode='train'):
        ...
        if mode == 'train':
            if pred.shape[1] == 1: # gray
                ...
                pred_grad = torch.concat([diff_operators.gradient(pred[..., i], original_coordinate) for i in range(3)], dim=-1) # (h*w, c*2)
