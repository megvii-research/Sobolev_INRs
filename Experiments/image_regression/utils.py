...
def grad2rgb(grad, q=0.05):
    ...
    grad_hsv[:, 0] = (
        grad_angle + torch.pi
    )  # kornia.color.hsv_to_rgb assume Hue values are in the range [0 ~ 2pi]
    grad_hsv[:, 1] = 1.0

    per_min = torch.quantile(
        einops.rearrange(grad_mag, "b h w -> b (h w)"), q=q, dim=-1
    )
    per_max = torch.quantile(
        einops.rearrange(grad_mag, "b h w -> b (h w)"), q=1.0 - q, dim=-1
    )

    grad_mag = (grad_mag - per_min) / (per_max - per_min)
    grad_mag = torch.clip(grad_mag, 0.0, 1.0)
...
def cal_metric(gt, pred, factor=None):
    ...
    if gt.shape[1] == 3:  # color
        gt = (
            gt
            * torch.tensor([65.738, 129.057, 25.064])
            .div(256.0)
            .view(1, 3, 1, 1)
            .to(gt.device)
        ).sum(
    ...
    if factor == None:  # train
        ...
        ssim = cal_ssim(
            gt[..., factor:-factor, factor:-factor],
            pred[..., factor:-factor, factor:-factor],
        )  # crop image following common settings of super resolution.
...
def write_train_summary(data, output, writer, epoch, shape, out_train_imgs_dir):
    ...
    gt = einops.rearrange(gt, "(h w) c -> c h w", h=shape.height).unsqueeze(
        0
    )  # (1, c, h, w)
    pred = output["pred"]
    ...
    cv2.imwrite(
        os.path.join(out_train_imgs_dir, "gt_vs_pred" + "-" + "%05d" % epoch + ".png"),
        (
            255
            * einops.rearrange(torch.flip(gt_vs_pred, dims=[0]), "c h w -> h w c")
            .cpu()
            .numpy()
        ).astype(np.uint8),
    )
    ...
    gt_grad = einops.rearrange(gt_grad, "(h w) t -> h w t", h=shape.height).unsqueeze(
        0
    )  # (1, h, w, c*2)
    pred_grad = output["pred_grad"]
    pred_grad = einops.rearrange(
        pred_grad, "(h w) t -> h w t", h=shape.height
    ).unsqueeze(0)

    if output["pred"].shape[1] == 1:  # gray
        gt_grad_rgb = grad2rgb(gt_grad)  # (b, c, h, w)
        pred_grad_rgb = grad2rgb(pred_grad)
        gt_vs_pred_grad = torch.cat(
            [gt_grad_rgb, pred_grad_rgb], dim=-1
        )  # (b, c, h, w*2)
        gt_vs_pred_grad = make_grid(
            gt_vs_pred_grad, scale_each=True, normalize=True
        )  # (c, h, w*2)
        writer.add_image(prefix + "gt_vs_pred_grad", gt_vs_pred_grad, global_step=epoch)
        cv2.imwrite(
            os.path.join(
                out_train_imgs_dir, "gt_vs_pred_grad" + "-" + "%05d" % epoch + ".png"
            ),
            (
                255
                * einops.rearrange(
                    torch.flip(gt_vs_pred_grad, dims=[0]), "c h w -> h w c"
                )
                .cpu()
                .numpy()
            ).astype(np.uint8),
        ...
        for i, c in enumerate(["r", "g", "b"]):
            ...
            writer.add_image(
                prefix + c + "_gt_vs_pred_grad", gt_vs_pred_grad, global_step=epoch
            )
            cv2.imwrite(
                os.path.join(
                    out_train_imgs_dir,
                    c + "_gt_vs_pred_grad" + "-" + "%05d" % epoch + ".png",
                ),
                (
                    255
                    * einops.rearrange(
...
def write_test_summary(data, output, writer, epoch, shape, out_test_imgs_dir, factor):
    ...
    cv2.imwrite(
        os.path.join(out_test_imgs_dir, "gt_vs_pred" + "-" + "%05d" % epoch + ".png"),
        (
            255
            * einops.rearrange(torch.flip(gt_vs_pred, dims=[0]), "c h w -> h w c")
            .cpu()
