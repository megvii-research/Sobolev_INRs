...
rounding_half_up_4 = lambda x: Decimal(str(x)).quantize(Decimal("0.0001"), rounding="ROUND_HALF_UP")
...
def eval_testset(testset_gt, testset_pred, f=None):
    ...
    for i, (gt, pred) in enumerate(zip(testset_gt, testset_pred)):
        ...
        print("%03d.png PSNR: %s, SSIM: %s" % (i, rounding_half_up_4(psnr), rounding_half_up_4(ssim)))
        print("%03d.png PSNR: %s, SSIM: %s" % (i, rounding_half_up_4(psnr), rounding_half_up_4(ssim)), file=f)
        psnr_list.append(psnr)
        ssim_list.append(ssim)
    print("Mean PSNR: %s, Mean SSIM: %s" % (rounding_half_up_4(np.mean(psnr_list)), rounding_half_up_4(np.mean(ssim_list))))
    print("Mean PSNR: %s, Mean SSIM: %s" % (rounding_half_up_4(np.mean(psnr_list)), rounding_half_up_4(np.mean(ssim_list))), file=f)
...
def eval_exp():
    ...
    for testset_pred_dir in testset_pred_to_be_processed:
        testset_pred = [imageio.imread(fname).astype(np.float32) / 255. for fname in glob.glob(os.path.join(testset_pred_dir, '*.png'))]
        if len(testset_pred) != len(i_test):
            print("Evaluating: %s, the number of predicted images is wrong, skipping..." % (testset_pred_dir))
        else:
            score_file = os.path.join(testset_pred_dir, 'score.txt')
