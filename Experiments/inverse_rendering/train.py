...
def config_parser():
    ...
    parser.add_argument('--der_operator', type=str, default='sobel', choices=('sobel', 'diff'))
    ...
    parser.add_argument("--show_der_loss", action='store_true',
...
def train():
    ...
    # Load LLFF data
    images, poses, bds, render_poses, i_test = load_llff_data(args.datadir, args.llff_factor,
    ...
    grad = kornia.filters.spatial_gradient(einops.rearrange(torch.Tensor(images), 'b h w c -> b c h w'), mode=args.der_operator, order=1, normalized=True) # [B, C, 2, H, W], grad_x = grad[:, 0], grad_y = grad[:, 1]
    ...
    # Create nerf model
    render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer = create_tiny_nerf(args)
    global_step = start
    ...
    coordinate = torch.stack(
            torch.meshgrid(
                ...
                indexing='xy'),
            -1) # [H, W, 2]
    coordinate = coordinate.unsqueeze(0).expand(images.shape[0], -1, -1, -1) # [N, H, W, 2]

    downsampled_images = images[:, ::args.factor, ::args.factor, :] # [N, H//factor, W//factor, 3]
    print("Downsampled images shape: ", downsampled_images.shape)
    downsampled_coordinate = coordinate[:, ::args.factor, ::args.factor, :] # [N, H//factor, W//factor, 2]
    downsampled_grad = grad[:, ::args.factor, ::args.factor, :] # [N, H//factor, W//factor, 3*2]

    poses = torch.Tensor(poses).to(device) # [N, 3, 4]
    data = torch.cat([downsampled_images, downsampled_coordinate, einops.rearrange(poses, 'n e f -> n () () (e f)').expand(-1, H//args.factor, W//args.factor, -1), downsampled_grad], dim=-1) # [B, H//factor, W//factor, 3+2+12+c*2], (rgb, xy, pose, grad)
    data = torch.stack([data[i] for i in i_train]) # [N_train, H//factor, W//factor, 3+2+12+3*2]
    data = einops.rearrange(data, 'n h w c -> (n h w) c') # [N_train*(H//factor)*(W//factor), 3+2+12+3*2], (rgb, xy, pose, grad)
    ...
    for i in trange(start, N_iters):
        ...
        batch_data = data[i_batch:i_batch+N_rand] # [N_train*H*W, 3+2+12+c*2], (rgb, xy, pose, grad)
        target_s = batch_data[:, :3]
        coordinate_s = batch_data[:, 3:5].float().clone().detach().requires_grad_(True) # enable us to compute gradients w.r.t. coordinates(xy) 
        ...
        if args.supervision == 'val':
            ...
            der_loss = der_mse(rgb, coordinate_s, target_grad_s)
            loss = 1. * val_loss + args.lambda_der * der_loss 
